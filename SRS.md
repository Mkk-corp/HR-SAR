...
**شجرة الهرم (`facilityTreeHTML(f)`):**
- تُستدعى داخل `renderFacilityDetail()` وتُدمج في `#facilityInfoCard`
- الشرط: منشأة أساسية لها أبناء، أو منشأة فرعية لها أب مسجَّل
- **أساسية:** الجذر = المنشأة نفسها + أفرع = جميع من `parentId === f.id`
- **فرعية:** الجذر = الأب + أفرع = جميع من `parentId === f.parentId`
- المنشأة الحالية تُمييَّز بـ `.fac-tree-current` وشارة "الحالي"
